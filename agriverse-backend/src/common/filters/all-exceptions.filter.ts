...
function toVnEn(messageVn: string, messageEn: string) {
  return `${messageVn} (${messageEn})`;
}
...
function normalizeHttpExceptionMessage(exception: HttpException): string {
  ...
  switch (exception.getStatus()) {
    case HttpStatus.UNAUTHORIZED:
      return toVnEn('Không có quyền truy cập', 'Unauthorized');
    case HttpStatus.FORBIDDEN:
      return toVnEn('Không có quyền thực hiện', 'Forbidden');
    case HttpStatus.NOT_FOUND:
      return toVnEn('Không tìm thấy', 'Not found');
    case HttpStatus.BAD_REQUEST:
      return toVnEn('Dữ liệu không hợp lệ', 'Bad request');
    default:
      return toVnEn('Có lỗi xảy ra', 'An error occurred');
  }
}

function normalizePrismaErrorMessage(exception: unknown): string | null {
  if (exception instanceof Prisma.PrismaClientKnownRequestError) {
    switch (exception.code) {
      case 'P2002':
        return toVnEn('Dữ liệu đã tồn tại', 'Unique constraint failed');
      case 'P2003':
        return toVnEn(
          'Không thể thực hiện vì vẫn còn dữ liệu liên quan',
          'Foreign key constraint failed',
        );
      default:
        return toVnEn('Lỗi cơ sở dữ liệu', 'Database error');
    }
  }

  if (exception instanceof Prisma.PrismaClientValidationError) {
    return toVnEn('Dữ liệu không hợp lệ', 'Validation error');
  }
...
export class AllExceptionsFilter implements ExceptionFilter {
  constructor(private readonly httpAdapterHost: HttpAdapterHost) {}
    ...
    const prismaMsg = normalizePrismaErrorMessage(exception);
    ...
    const message =
      exception instanceof HttpException
        ...
        : prismaMsg ?? toVnEn('Lỗi máy chủ nội bộ', 'Internal server error');
